...
// one row
if(width()>1155&&!b.length)c.append(d.children()).removeClass("row-1").addClass("row-0"),d.remove();else if(b.length){var e=b.children("td").slice(0,3),f=b.children("td").slice(3,6);a.append($("<tr/>",{"class":"row-1",html:e})).append($("<tr/>",{"class":"row-2",html:f})),b.remove()}}function checkboxResize(){var a=$("table.rx").not(".hidden"),b=a.find(".row-0");
// 2 rows
...
return 9==b&&c==d?(a.preventDefault(),void $("#add").trigger("click")):13==b?(a.preventDefault(),void $(this).blur()):void 0}}),$("[id$=_dose], [id$=_days], [id$=_mg], [id$=_time]").each(function(a){var b=$(this),c=b[0].id;b.onFocus(),b.onBlur(),~c.indexOf("dose")?b.doseInput():~c.indexOf("days")?b.daysInput():~c.indexOf("mg")?(b.doseInput(),b.linearInput()):~c.indexOf("time")&&(b.daysInput(),b.linearInput())}),$("#add, #ladd").click(function(a){$(this).closest("form").find(":input").slice(-1).blur(),"ladd"==a.target.id&&(/*         $('#ldel')
