...
B[/ut|nd/.test(c)?_remove:_add](w)),!_mobile)return!1;a.stopPropagation()}})})}}(window.jQuery||window.Zepto),$(document).ready(function(){if($("form#nl_taper").floatlabels({style:1}),/* datepicker (pickadate)
    ---------------------------------------------------------------------------*/
date_picker(),mobile?($(".picker__frame").css({bottom:"0","margin-bottom":"0",top:"auto"}),$(".picker__box").css({"border-radius":"0","border-bottom-width":"0"})):$(".picker__frame").css({top:"15%"}),mobile){var a=navigator.userAgent.toLowerCase(),b=a.match(/(iPad|iPhone|iPod)/i);b&&(
//console.log('localStorage.getItem("reload")', localStorage.getItem('reload'));
localStorage.getItem("reload")?(localStorage.removeItem("reload"),console.log('localStorage.getItem("reload")',localStorage.getItem("reload"))):(/* set reload locally and then reload the page */
localStorage.setItem("reload","true"),console.log('localStorage.getItem("reload")',localStorage.getItem("reload")),location.reload()))}
