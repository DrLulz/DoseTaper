...
B.length&&v&&d==s&&
// mouseout|touchend
B[/ut|nd/.test(c)?_remove:_add](w)),!_mobile)return!1;a.stopPropagation()}})})}}(window.jQuery||window.Zepto),$(document).ready(function(){$("form#nl_taper").floatlabels({style:1});var a=$(".input-group-addon"),b=$("#input_date").pickadate({format:"mmmm dd, yyyy",today:'<i class="fa fa-crosshairs" aria-hidden="true"></i>',clear:"",close:'<i class="fa fa-times" aria-hidden="true"></i>',onStart:function(){var a=new Date;this.set("select",[a.getFullYear(),a.getMonth(),a.getDate()])},onClose:function(){$(".date-wrapper").removeClass("date-hover")}}),c=b.pickadate("picker");if(a.on("click",function(a){c.get("open")?c.close():c.open(),a.stopPropagation()}).on("mousedown",function(a){a.preventDefault()}),mobile?($(".picker__frame").css({bottom:"0","margin-bottom":"0",top:"auto"}),$(".picker__box").css({"border-radius":"0","border-bottom-width":"0"})):$(".picker__frame").css({top:"15%"}),$(".checkbox:not(.noiCheck)").iCheck({checkboxClass:"icheckbox_flat-grey",radioClass:"iradio_flat-grey",increaseArea:"20%",ifChecked:function(){console.log($(this).next())}}),$(".checkbox:not(.noiCheck)").on("ifChecked",function(a){$(this).parent().next().addClass("checked")}).on("ifUnchecked",function(a){$(this).parent().next().removeClass("checked")}),
...
$(".date-wrapper").mouseover(function(){$(this).addClass("date-hover")}).mouseout(function(){$(this).removeClass("date-hover")}),$(window).width()>649&&$(window).width()<980){var d=$(".row-1").html(),e=$(".row-2").html();$(".size-param tbody").detach(),$(".size-param table").append($("<tr/>",{"class":"row-0",html:d+e})).html()}$(window).resize(function(){var a=$(window).width();console.log(a);var b=$(".size-param table"),c=$(".size-param tbody"),d=$(".row-1").html(),e=$(".row-2").html();if(a>649&&a<980)$(".row-0").length||(console.log("! .row-0"),c.detach(),b.append($("<tr/>",{"class":"row-0",html:d+e})).html());else if((a<650||a>779)&&$(".row-0").length){var f=$("tr.row-0").children("td").slice(0,3),g=$("tr.row-0").children("td").slice(3,6);c.detach(),b.append($("<tr/>",{"class":"row-1",html:f})),b.append($("<tr/>",{"class":"row-2",html:g}))}})}),
//(function($){
