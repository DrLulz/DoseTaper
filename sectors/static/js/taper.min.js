...
B.length&&v&&d==s&&
// mouseout|touchend
B[/ut|nd/.test(c)?_remove:_add](w)),!_mobile)return!1;a.stopPropagation()}})})}}(window.jQuery||window.Zepto),$(document).ready(function(){if($("form#nl_taper").floatlabels({style:1}),/* datepicker (pickadate)
    ---------------------------------------------------------------------------*/
date_picker(),mobile?($(".picker__frame").css({bottom:"0","margin-bottom":"0",top:"auto"}),$(".picker__box").css({"border-radius":"0","border-bottom-width":"0"})):$(".picker__frame").css({top:"15%"}),mobile){var a=navigator.userAgent.toLowerCase(),b=a.match(/(iPad|iPhone|iPod)/i);b&&(console.log('localStorage.getItem("reload")',localStorage.getItem("reload")),localStorage.getItem("reload")?(localStorage.removeItem("reload"),console.log('localStorage.getItem("reload")',localStorage.getItem("reload"))):(/* set reload locally and then reload the page */
localStorage.setItem("reload","true"),console.log('localStorage.getItem("reload")',localStorage.getItem("reload"))))}
// Taper start date, grow/shrink input box to text
$("input.autogrow").autoGrowInput({minWidth:30,comfortZone:20}),$(window).resize(function(){$("input.autogrow").autoGrowInput({minWidth:30,comfortZone:20})}),
...
return console.log("viewportW()",verge.viewportW()),a=$.extend({
//maxWidth: 1000,
maxWidth:verge.viewportW(),minWidth:0,comfortZone:70},a),this.filter("input:text").each(function(){
//console.log('$(this).width()', $(this).width());
var b=a.minWidth||$(this).width(),c="",d=$(this),e=$("<tester/>").css({position:"absolute",top:-9999,left:-9999,width:"auto",fontSize:d.css("fontSize"),fontFamily:d.css("fontFamily"),fontWeight:d.css("fontWeight"),letterSpacing:d.css("letterSpacing"),whiteSpace:"nowrap"}),f=function(){if(c!==(c=d.val())){
