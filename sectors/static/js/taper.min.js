...
/* f(x)
---------------------------------------------------------------------------*/
function paramsSize(){$(window).width()<lg&&$(".med-wrapper, .size-wrapper").removeClass("v-align"),$(window).resize(function(){var a=$(window).width();a<lg?$(".med-wrapper, .size-wrapper").removeClass("v-align"):a>lg&&$(".med-wrapper, .size-wrapper").addClass("v-align")})}// taperControls()
function date_picker(){var a=function(){$("#input_date").text(d.get())},b=$(".input-group-addon"),c=$("#input_date").pickadate({format:"mmmm dd, yyyy",today:'<i class="fa fa-crosshairs" aria-hidden="true"></i>',clear:"",close:'<i class="fa fa-times" aria-hidden="true"></i>',formatSubmit:"mm/dd/yyyy",hiddenName:!0,onStart:function(){var a=new Date;this.set("select",[a.getFullYear(),a.getMonth(),a.getDate()])},onClose:function(){a(),$(".cal-wrapper").removeClass("cal-hover")}}),d=c.pickadate("picker");b.on("click",function(a){d.get("open")?d.close():d.open(),a.stopPropagation()}).on("mousedown",function(a){a.preventDefault()}),a()}// datepicker()
...
function checkbox_resize(){$(window).width()>649&&$(window).width()<980&&($(".row-1").append($(".row-2").children()).removeClass("row-1").addClass("row-0"),$(".row-2").remove()),$(window).resize(function(){var a=$(window).width(),b=$(".size-param table"),c=$(".row-0"),d=$(".row-1"),e=$(".row-2");if(a>649&&a<980)c.length||(d.append(e.children()).removeClass("row-1").addClass("row-0"),e.remove());else if((a<650||a>779)&&c.length){var f=$("tr.row-0").children("td").slice(0,3),g=$("tr.row-0").children("td").slice(3,6);b.append($("<tr/>",{"class":"row-1",html:f})),b.append($("<tr/>",{"class":"row-2",html:g})),c.remove()}})}/* f(x)
---------------------------------------------------------------------------*/
...
// Data API //
//////////////
a("[data-headroom]").each(function(){var b=a(this);b.headroom(b.data())}))}(window.Zepto||window.jQuery),$.fn.reconstitute=function(){},$.fn.taperControls=function(){var a=$("<div/>",{"class":"phase-add-delete"}).append($("<div>",{id:"del","class":"phase-del"})).append($("<div>",{id:"add","class":"phase-add"})).append($("<div>",{id:"calc","class":"taper-calc"}));this.append(a),$(".phase-add, .phase-del, .taper-calc").on("mouseover",function(){$(this).filter(":not(:animated)").animate({width:"100%"},"fast")}).on("mouseout",function(){$(this).animate({width:"80%"},"fast")})},// checkbox_resize()
/* taper js
---------------------------------------------------------------------------*/
/* floating labels
--------------------------------------------------*/
$("#nl_taper form").floatlabels({customEvent:null,customLabel:null,exclude:".no-label",inputRegex:/email|number|password|search|tel|text|url/,prioritize:"label",requiredClass:"required",style:1,transform:"input, select, textarea"}),/* datepicker (pickadate)
--------------------------------------------------*/
date_picker(),
...
$(".cal-wrapper").mouseover(function(){$(this).addClass("cal-hover")}).mouseout(function(){$(this).removeClass("cal-hover")}),/* checkboxes (iCheck)
--------------------------------------------------*/
checkboxes(),checkbox_resize(),/* headroom js
--------------------------------------------------*/
...
--------------------------------------------------*/
$(".phase").last().taperControls(),/* resize taper params on large screens
--------------------------------------------------*/
paramsSize(),/* Q.O.D. behavior
--------------------------------------------------*/
$(".phase-qod").on("click",function(a){$(this).toggleClass("active");var b=0==$(this).children().val()?"1":"0";$(this).children().val(b),a.preventDefault()}),/* Dynamic Phases
---------------------------------------------------------------------------*/
$(function(){$("#add").click(function(){
// increment phase number
var a=1;$.each($(".phase"),function(){parseInt($(this).data("id"))>a&&(a=parseInt($(this).data("id")))}),a++;var b=a,c="p"+a,d=$("<div/>",{id:c,"data-id":b,"class":"row phase animated slideInRight"}),e=$("<div/>",{"class":"phase-label",unselectable:"on",html:"Phase "+b});e.appendTo(d);var f=$("<div/>",{"class":"phase-qod-wrapper"}).append($("<div/>",{"class":"phase-qod"}).append($("<input/>",{"class":"phase-qod",id:"nl_qod_"+b,name:"nl_qod_"+b,type:"hidden",value:"0"})));f.appendTo(d);var g=$("<div/>",{"class":"col-xs-2 phase-num v-align"}).append($("<p/>",{unselectable:"on",html:b}));g.appendTo(d),
//$('<div class="col-xs-2 col-md-1 phase-spacer"></div>').appendTo($phase);
$("<div/>",{"class":"col-xs-2 col-md-1 phase-spacer"}).appendTo(d);var h=$("<div/>",{"class":"col-xs-8 col-md-9 phase-params"}),i=$("<div/>",{"class":"row"}),j=$("<div/>",{"class":"col-md-6 dose-param"}).append($("<label/>",{"class":"param-label","for":c+"_dose",html:"Dose"})).append($("<input/>",{"class":"param-input",type:"tel",id:c+"_dose",name:c+"_dose"})),k=$("<div/>",{"class":"col-md-6 days-param"}).append($("<label/>",{id:c+"_days_label","class":"param-label","for":c+"_days",html:"Days"})).append($("<input/>",{"class":"param-input",type:"tel",id:c+"_days",name:c+"_days"}));j.appendTo(i),k.appendTo(i),i.appendTo(h),h.appendTo(d),d.appendTo($("#nl_form .container-fluid")),$("html, body").animate({scrollTop:d.offset().top},500)}),$("#del").click(function(){var a=$(".phase").slice(2).last();a.slideToggle("fast","linear",function(){a.remove()})}),$("#nl_form").on("keydown","[id$=_days]:last",function(a){9==a.which&&$("#add").trigger("click")})}),jQuery(function(a){var b=a("#p1_dose");b.keypress(function(a){return isDose(a,this)}),b.on("change",function(){var b=parseFloat(a(this).val()),c=b+" mg";a(this).val(c)});var c=a("#p1_days");c.keypress(function(a){return isDays(a,this)}),c.on("change",function(){var b=parseFloat(a(this).val()),c=b+" days";a(this).val(c);var d=Math.floor(parseInt(b,10)/7),e=parseInt(b,10)%7;if(0!=d)var f=d+" Weeks, "+e+" Days";else var f="Days "+e;a("#p1_days_label").html(f)})});