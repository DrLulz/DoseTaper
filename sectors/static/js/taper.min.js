 ...
 */
/** global: Option */
function onOpened(a){console.log("ON onSelected()")}function onSelected(a,b){
//console.log('$e', $e.target)
//console.log('rx_name', rx_name)
$(".meds").not("#"+b).hide(),$("#"+b).show(),a.target.blur()}function onAutocompleted(a,b){
//console.log('rx_name', rx_name)
$(".meds").not("#"+b).hide(),$("#"+b).show(),a.target.blur()}
...
//var date = new Date();
var a=new Date(2016,11,31);this.set("select",[a.getFullYear(),a.getMonth(),a.getDate()])},onClose:function(){a(),$(".cal-wrapper").removeClass("cal-hover")}}),d=c.pickadate("picker");b.on("click",function(a){d.get("open")?d.close():d.open(),a.stopPropagation()}).on("mousedown",function(a){a.preventDefault()}),a()}/* checkboxes (iCheck)
---------------------------------------------------------------------------*/
function checkBoxes(){$(".checkbox:not(.noiCheck)").iCheck({checkboxClass:"icheckbox_flat-grey",radioClass:"iradio_flat-grey",
//increaseArea: '20%',
ifChecked:function(){console.log($(this).next())}}),$(".checkbox:not(.noiCheck)").on("ifChecked",function(a){$(this).parent().next().addClass("checked")}).on("ifUnchecked",function(a){$(this).parent().next().removeClass("checked")}),$(".checkbox.qod:not(.noiCheck)").iCheck({checkboxClass:"icheckbox_futurico",radioClass:"iradio_futurico",increaseArea:"20%",ifChecked:function(){}}),$(".checkbox.qod:not(.noiCheck)").on("ifChecked",function(a){$(this).parent().next().addClass("checked")}).on("ifUnchecked",function(a){$(this).parent().next().removeClass("checked")})}function checkboxResize(){
// < 1175 go to 2 rows
// 650 to 
// < 650 go to 2 rows
// < 980 = 1
// < 650 = 2
if($(window).width()<733||$(window).width()>779){var a=$("tr.row-0").children("td").slice(0,3),b=$("tr.row-0").children("td").slice(3,6);$(".size-param table").append($("<tr/>",{"class":"row-1",html:a})),$(".size-param table").append($("<tr/>",{"class":"row-2",html:b})),$(".row-0").remove()}$(window).resize(function(){var a=$(window).width(),b=$(".size-param table"),c=$(".row-0"),d=$(".row-1"),e=$(".row-2");
// one row
//if ( width > 732 && width < 980) {
if(a>1155||a<980&&a>732)c.length||(d.append(e.children()).removeClass("row-1").addClass("row-0"),e.remove());else if((a<733||a>779)&&c.length){var f=$("tr.row-0").children("td").slice(0,3),g=$("tr.row-0").children("td").slice(3,6);b.append($("<tr/>",{"class":"row-1",html:f})),b.append($("<tr/>",{"class":"row-2",html:g})),c.remove()}})}/* resize taper params on large screens
---------------------------------------------------------------------------*/
function paramsSize(){$(window).width()<lg&&$(".med-wrapper, .size-wrapper").removeClass("v-align"),$(window).resize(function(){var a=$(window).width();a<lg?$(".med-wrapper, .size-wrapper").removeClass("v-align"):a>lg&&$(".med-wrapper, .size-wrapper").addClass("v-align")})}// paramsSize()
//paramsSize();
/* Q.O.D. behavior
---------------------------------------------------------------------------*/
function phaseQod(){$("[id$=_taper]").on("click",".phase-qod",function(a){a.stopPropagation(),a.stopImmediatePropagation(),$(this).toggleClass("active");var b=0==$(this).children().val()?"1":"0";$(this).children().val(b),a.preventDefault()})}/* f(x)
---------------------------------------------------------------------------*/
...
$.each(a,function(a,b){substrRegex.test(b)&&d.push(b)}),c(d)}},b=["Prednisone","Methylprednisolone"],c=$("#taper_med").typeahead({hint:!0,highlight:!0,minLength:0},{name:"prescription",source:a(b)}).bind("focus",function(){
//console.log('focus', $(this).val())
$(this).data("initial_value",$(this).val()),c.typeahead("val","")}).bind("blur",function(){var a=$(this).data("initial_value"),b=$(this).val();
//console.log('saved_value', saved_value)
//console.log('new_value', new_value)
b?c.typeahead("val",b):c.typeahead("val",a)})}),$(".typeahead").on("typeahead:opened",onOpened).on("typeahead:selected",onSelected).on("typeahead:autocompleted",onAutocompleted),$("[id$=_form]").on("keydown",".twitter-typeahead #taper_med",function(a){13!=a.which&&9!=a.which||(a.preventDefault(),$(".tt-selectable").first().click())}),doseParam(),daysParam(),// datepicker()
datePicker(),
...
$(".cal-wrapper").mouseover(function(){$(this).addClass("cal-hover")}).mouseout(function(){$(this).removeClass("cal-hover")}),// checkboxes()
checkBoxes(),// checkbox_resize()
checkboxResize(),/* headroom js
---------------------------------------------------------------------------*/
...
---------------------------------------------------------------------------*/
$(function(){$("#add").click(function(){$(this).closest("form").find(":input").slice(-1).blur();
...
var h=$("<div/>",{"class":"col-xs-8 pull-xs-2 col-md-9 pull-md-1 phase-params"}),i=$("<div/>",{"class":"row"}),j=$("<div/>",{"class":"col-md-6 dose-param"}),k=$("<label/>",{"class":"param-label","for":c+"_dose",html:"Dose"}),l=$("<input/>",{"class":"param-input",type:"tel",id:c+"_dose",name:c+"_dose"});k.appendTo(j),l.appendTo(j);
// days
